...
import { likeVideo, unlikeVideo, saveVideo, shareVideo, checkVideoLiked } from "@/lib/api"
import { formatTimeAgo, formatCount } from "@/lib/utils"; // Import for formatting time and count
...
export default function VideoCard({ video }) {
  ...
  useEffect(() => {
    const checkLikeStatus = async () => {
      if (user) {
        ...
        const isLiked = await checkVideoLiked(videoId);
        setLiked(isLiked);
      } else {
        setLiked(false);
      }
    ...
    // Nếu likesCount chưa được set (lần đầu), dùng giá trị từ video
    // Nếu đã có interaction, likesCount sẽ được update bởi state local, 
    // nhưng ở đây ta reset khi video prop thay đổi
  ...
  const handleLike = async () => {
    ...
    try {
      ...
      if (newLikedState) {
        response = await likeVideo(videoId);
        // Cập nhật lại UI dựa trên phản hồi từ server
        setLiked(true);
        if (response.likes !== undefined) setLikesCount(response.likes);
      } else {
        response = await unlikeVideo(videoId);
        // Cập nhật lại UI dựa trên phản hồi từ server
        setLiked(false);
      }
    ...
    <div className="border border-gray-200 rounded-lg overflow-hidden sm:max-w-lg md:max-w-xl lg:max-w-2xl mx-auto">      <div className="p-3 sm:p-4 flex items-center">
      ...
      <div className={`relative ${isMobile && orientation === 'landscape' ? 'fixed inset-0 z-50 bg-black flex items-center justify-center' : ''}`}>
        ...
        >
          <button
            onClick={handleLike}
            className={`bg-gray-800 bg-opacity-50 rounded-full ${isMobile && orientation === 'portrait' ? "p-1.5" : "p-2"
